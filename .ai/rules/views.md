...
## Popular section: automatic from order_items, pending-only, auto-hide
Popular Products (জনপ্রিয় পণ্য) is placed between Seasonal/এ সময়ের পণ্য and the Footer (spec §29). Popularity is AUTOMATIC from real purchase data: HomepageService::popularProducts() aggregates SUM(order_items.quantity) joined variant→product, restricted to orders with status = Order::STATUS_PENDING (the only order status that exists in this app — no cancelled/failed/completed states yet; add them to the filter when the order workflow grows). Filters products.is_active=1 + not soft-deleted in the aggregate (no PHP-side order loop — pure DB aggregation), tie-break by product_id, limit config('shop.homepage.popular_limit') (default 4). Empty collection (no purchase data) hides the whole section — never fabricate popularity or fall back to random products. Do NOT add a "জনপ্রিয়" badge to the shared product-card (it would leak into every section) — the section heading communicates popularity.

## New Arrivals: automatic newest-active, not admin-flag gated
New Arrivals (নতুন যোগ করা পণ্য) is placed between Popular Products and the Footer (spec §25). It is fully AUTOMATIC: HomepageService::newArrivals() = productQuery()->latest() (created_at DESC, no published_at exists) limited by config('shop.homepage.new_arrivals_limit') (default 4). It does NOT depend on the is_new_arrival admin flag — that flag is a separate optional merchandising badge shown by the existing product-card, not the section selector (admin must not manually maintain this section). Active/soft-delete visibility comes from the shared productQuery()/active() scope. Hide the whole section when empty. Do NOT invent a per-product "নতুন" badge or an arbitrary date window.

## Contact CTA: env config-driven, hide when nothing configured
Quick Contact CTA (কোনো কিছু জানতে চান?) sits between New Arrivals and the Footer (spec §23 flow). It is config-driven, NOT a settings table: HomepageService::contactActions() reads config('shop.contact') (phone / whatsapp / whatsapp_url / email, all .env-driven via SHOP_*) and returns only actions whose value is set. whatsapp_url is preferred; otherwise a wa.me link is built from the whatsapp number. A tel:/mailto: link is built from phone/email. Primary button (যোগাযোগ করুন) only renders when a contact route EXISTS (Route::has('contact')) — the app currently has NO contact page, so it stays hidden. The section itself only renders when at least one real action exists — never show placeholder numbers/emails (footer's contact@example.com is a `   placeholder`; do not source from it. .env.example documents the SHOP_* keys. All contact config values are currently empty.
